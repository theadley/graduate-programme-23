...
export function getCountryCodeByNationality(nationality: string): string {
  if (!nationality) return 'UNK';
  return nationalities.find(nat => nat.Nationality === nationality)?.CountryCode ?? 'UNK';
}
